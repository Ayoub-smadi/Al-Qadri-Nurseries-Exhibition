...
const categories: { id: AgriStoreCategory; ar: string }[] = [
  ...
  { id: 'pesticides', ar: 'مبيدات' }, { id: 'irrigation', ar: 'شبكات ري' }, { id: 'seedling-supplies', ar: 'مستلزمات تشتيل' },
];
...
export function AdminAgriProductsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const saveZone = () => {
    ...
    setDraft(prev => ({ ...prev, shippingZones: [...(prev.shippingZones ?? []), zone] }));
    setNewZone({ nameAr: '', fee: '' });
  };
  const saveAllData = () => {
    updateSiteData({
      agriStoreContent: draft.agriStoreContent,
      ...
      <div className="p-4 border-b flex justify-between items-center"><div><h2 className="font-bold arabic">إدارة منتجات المواد الزراعية</h2><p className="text-xs text-muted-foreground arabic">أضف المنتجات مع صورها وأسعارها ليظهر السعر جاهزاً للعميل</p></div><button onClick={onClose}><X className="w-5 h-5" /></button></div>
       <div className="p-4 space-y-3">
         <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 space-y-3">
           ...
           <input value={draft.agriStoreContent?.storeTitleAr ?? ''} onChange={e => setDraft(prev => ({ ...prev, agriStoreContent: { ...prev.agriStoreContent!, storeTitleAr: e.target.value } }))} placeholder="اسم المتجر" className="w-full rounded-xl border bg-background p-2.5 arabic" />
           <input value={draft.agriStoreContent?.storeSubtitleAr ?? ''} onChange={e => setDraft(prev => ({ ...prev, agriStoreContent: { ...prev.agriStoreContent!, storeSubtitleAr: e.target.value } }))} placeholder="العنوان الفرعي" className="w-full rounded-xl border bg-background p-2.5 arabic" />
           <input value={draft.agriStoreContent?.eyebrowAr ?? ''} onChange={e => setDraft(prev => ({ ...prev, agriStoreContent: { ...prev.agriStoreContent!, eyebrowAr: e.target.value } }))} placeholder="الجملة الصغيرة" className="w-full rounded-xl border bg-background p-2.5 arabic" />
           <input value={draft.agriStoreContent?.titleAr ?? ''} onChange={e => setDraft(prev => ({ ...prev, agriStoreContent: { ...prev.agriStoreContent!, titleAr: e.target.value } }))} placeholder="عنوان المتجر" className="w-full rounded-xl border bg-background p-2.5 arabic" />
           <textarea value={draft.agriStoreContent?.descriptionAr ?? ''} onChange={e => setDraft(prev => ({ ...prev, agriStoreContent: { ...prev.agriStoreContent!, descriptionAr: e.target.value } }))} placeholder="وصف المتجر" className="w-full rounded-xl border bg-background p-2.5 arabic" />
         </div>
        ...
        {editing && <div className="border-t pt-4 space-y-3">
         ...
         <div className="border-t pt-4 space-y-3">
           <div><h3 className="font-bold arabic">مناطق التوصيل ورسوم الشحن</h3><p className="text-xs text-muted-foreground arabic">لا تُضاف الرسوم للطلب إلا بعد اختيار العميل لمنطقة.</p></div>
            {zones.map(zone => <div key={zone.id} className="flex items-center gap-2 border rounded-xl p-2"><span className="flex-1 arabic">{zone.nameAr}</span><input type="number" min="0" step="0.01" value={zone.fee} onChange={e => setDraft(prev => ({ ...prev, shippingZones: (prev.shippingZones ?? []).map(item => item.id === zone.id ? { ...item, fee: Number(e.target.value) } : item) }))} className="w-24 rounded-lg border bg-background p-2 text-sm" /><span className="text-xs">د.أ</span><button onClick={() => setDraft(prev => ({ ...prev, shippingZones: (prev.shippingZones ?? []).filter(item => item.id !== zone.id) }))} className="p-2 text-destructive"><Trash2 className="w-4 h-4" /></button></div>)}
