  ...
  const handlePdf = async (fitToSinglePage = false) => {
    ...
    try {
      ...
      paper.querySelectorAll("input, textarea").forEach((node) => {
        ...
        const isWrappedTableCell = input.getAttribute("aria-label") === "صنف البند"
          || input.getAttribute("aria-label") === "ملاحظات البند";
        ...
        div.style.cssText = [
          ...
          "overflow-wrap:anywhere", `line-height:${isWrappedTableCell ? "1.55" : style.lineHeight}`,
          `min-height:${input instanceof HTMLTextAreaElement ? "0" : style.height}`,
          `padding:${isWrappedTableCell ? "3px 0 5px" : style.padding}`,
        ].join(";");
                    ...
                    {draft.rows.map((row) => (
                      <tr key={row.id}>
                        ...
                        <td style={{ border: "1px solid #27382e", padding: 4 }}><EditableField value={row.totalDinar} onChange={(value) => updateRow(row.id, "totalDinar", value)} ariaLabel="دينار البند" inputMode="decimal" style={{ textAlign: "center", fontSize: 11 }} /></td>
                         <td style={{ border: "1px solid #27382e", padding: 4, verticalAlign: "middle" }}><EditableField value={row.product} onChange={(value) => updateRow(row.id, "product", value)} ariaLabel="صنف البند" placeholder="الصنف" style={{ textAlign: "right", fontSize: 11 }} /></td>
                        ...
                        <td style={{ border: "1px solid #27382e", padding: 4 }}><EditableField value={row.origin} onChange={(value) => updateRow(row.id, "origin", value)} ariaLabel="منشأ البند" style={{ textAlign: "center", fontSize: 11 }} /></td>
                         <td style={{ border: "1px solid #27382e", padding: 4, verticalAlign: "middle" }}><EditableField value={row.notes} onChange={(value) => updateRow(row.id, "notes", value)} ariaLabel="ملاحظات البند" style={{ textAlign: "right", fontSize: 11 }} /></td>
