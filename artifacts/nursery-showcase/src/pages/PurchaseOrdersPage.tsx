...
const FORM_SETTINGS_KEY = "alqadri_purchase_order_form_settings";
const defaultSectionTitles = { request: "بيانات الطلب", buyer: "المشتري", supplier: "المورد", items: "الأصناف والخدمات", notes: "ملاحظات", buyerApproval: "اعتماد المشتري", supplierApproval: "اعتماد المورد" };
const defaultColumnTitles = { description: "وصف الصنف / الخدمة", unit: "الوحدة", quantity: "الكمية", unitPrice: "سعر الوحدة", total: "الإجمالي", origin: "المنشأ", notes: "ملاحظات" };
type FormSettings = { sectionTitles: typeof defaultSectionTitles; hiddenSections: Record<keyof typeof defaultSectionTitles, boolean>; columnTitles: typeof defaultColumnTitles; hiddenColumns: Record<keyof typeof defaultColumnTitles, boolean> };
const defaultFormSettings: FormSettings = { sectionTitles: defaultSectionTitles, hiddenSections: { request: false, buyer: false, supplier: false, items: false, notes: false, buyerApproval: false, supplierApproval: false }, columnTitles: defaultColumnTitles, hiddenColumns: { description: false, unit: false, quantity: false, unitPrice: false, total: false, origin: false, notes: false } };
...
function readFormSettings(): FormSettings {
  try { const saved = JSON.parse(localStorage.getItem(FORM_SETTINGS_KEY) || "null"); return { ...defaultFormSettings, ...saved, sectionTitles: { ...defaultSectionTitles, ...(saved?.sectionTitles || {}) }, hiddenSections: { ...defaultFormSettings.hiddenSections, ...(saved?.hiddenSections || {}) }, columnTitles: { ...defaultColumnTitles, ...(saved?.columnTitles || {}) }, hiddenColumns: { ...defaultFormSettings.hiddenColumns, ...(saved?.hiddenColumns || {}) } }; } catch { return defaultFormSettings; }
}
...
export default function PurchaseOrdersPage() {
  ...
  const [formSettings, setFormSettings] = useState<FormSettings>(() => readFormSettings());

  useEffect(() => { localStorage.setItem(DRAFT_KEY, JSON.stringify(order)); }, [order]);
  useEffect(() => { localStorage.setItem(FORM_SETTINGS_KEY, JSON.stringify(formSettings)); }, [formSettings]);
  ...
  const toggleSection = (key: keyof FormSettings["hiddenSections"]) => setFormSettings(prev => ({ ...prev, hiddenSections: { ...prev.hiddenSections, [key]: !prev.hiddenSections[key] } }));
  const setSectionTitle = (key: keyof FormSettings["sectionTitles"], value: string) => setFormSettings(prev => ({ ...prev, sectionTitles: { ...prev.sectionTitles, [key]: value } }));
  const toggleColumn = (key: keyof FormSettings["hiddenColumns"]) => setFormSettings(prev => ({ ...prev, hiddenColumns: { ...prev.hiddenColumns, [key]: !prev.hiddenColumns[key] } }));
  const setColumnTitle = (key: keyof FormSettings["columnTitles"], value: string) => setFormSettings(prev => ({ ...prev, columnTitles: { ...prev.columnTitles, [key]: value } }));
  const columnClass = (key: keyof FormSettings["hiddenColumns"]) => formSettings.hiddenColumns[key] ? "hidden" : "";
  ...
  return <div dir="rtl" className="min-h-screen bg-[#f4f7f5] text-slate-800 print:bg-white">
    <style>{`
      ...
      .po-section-title span { height:18px !important; width:3px !important; }
       ...
       .po-page label span { display:block; margin:0 !important; font-size:11.5px !important; line-height:1.4 !important; white-space:nowrap; overflow:visible; }
       .po-page .po-field, .po-page input, .po-page textarea { box-sizing:border-box; display:block; width:100%; font-size:12px !important; }
      ...
      .po-items-section table { min-width:0 !important; table-layout:fixed; font-size:10px !important; }
    ...
    {showCustomizer && <div className="no-print mx-auto mb-4 grid max-w-6xl gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-2">
      <div><h2 className="mb-3 text-lg font-black text-[#0d5c43]">الأقسام</h2><div className="grid gap-2 sm:grid-cols-2">{(Object.keys(defaultSectionTitles) as Array<keyof typeof defaultSectionTitles>).map(key => <div key={key} className="flex items-center gap-2"><input type="checkbox" checked={!formSettings.hiddenSections[key]} onChange={() => toggleSection(key)} /><Input value={formSettings.sectionTitles[key]} onChange={e => setSectionTitle(key, e.target.value)} className="h-9" /></div>)}</div></div>
      <div><h2 className="mb-3 text-lg font-black text-[#0d5c43]">أعمدة جدول الأصناف</h2><div className="grid gap-2 sm:grid-cols-2">{(Object.keys(defaultColumnTitles) as Array<keyof typeof defaultColumnTitles>).map(key => <div key={key} className="flex items-center gap-2"><input type="checkbox" checked={!formSettings.hiddenColumns[key]} onChange={() => toggleColumn(key)} /><Input value={formSettings.columnTitles[key]} onChange={e => setColumnTitle(key, e.target.value)} className="h-9" /></div>)}</div><p className="mt-3 text-xs text-slate-500">أزل علامة الصح لإخفاء القسم أو العمود. التعديلات تحفظ تلقائيًا.</p></div>
    </div>}
    <div className="po-preview">
    <main ref={paperRef} className={`po-page ${order.items.length > 8 ? "po-page-multipage" : ""}`}>
