...
import { ArrowRight, Eye, FilePlus2, Pencil, Printer, Save, Trash2, Plus, X } from "lucide-react";
...
export default function PurchaseOrdersPage() {
  ...
  return <div dir="rtl" className="min-h-screen bg-[#f4f7f5] text-slate-800 print:bg-white">
    <style>{`@media print { @page { size: A4 portrait; margin: 12mm; } .no-print { display:none!important } .po-page { padding:0!important; max-width:none!important } input,textarea { border:0!important; box-shadow:none!important; padding:0!important } section { break-inside: avoid; } }`}</style>
    <header className="no-print sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4">
      <div className="flex items-center gap-3"><Button variant="ghost" size="icon" onClick={() => navigate("/")} aria-label="العودة"><ArrowRight className="h-5 w-5" /></Button><div><p className="text-xs font-semibold text-[#0d5c43]">مؤسسة ومشاتل القادري الزراعية</p><h1 className="text-xl font-black text-slate-900">طلبات الشراء</h1></div></div>
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setShowList(true)} className="rounded-xl"><Eye className="ms-2 h-4 w-4" /> الطلبات المحفوظة</Button><Button variant="outline" onClick={createNew} className="rounded-xl"><FilePlus2 className="ms-2 h-4 w-4" /> طلب شراء جديد</Button><Button onClick={save} className="rounded-xl bg-[#0d5c43] hover:bg-[#084834]"><Save className="ms-2 h-4 w-4" /> حفظ الطلب</Button><Button onClick={() => window.print()} className="rounded-xl bg-slate-800 hover:bg-slate-700"><Printer className="ms-2 h-4 w-4" /> طباعة / حفظ PDF</Button></div>
    </div></header>

    <main className="po-page mx-auto max-w-6xl space-y-5 px-4 py-7">
      <div className="rounded-2xl bg-[#0d5c43] p-6 text-white shadow-lg print:rounded-none print:bg-white print:p-0 print:text-slate-900 print:shadow-none"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="mb-2 text-sm font-semibold text-emerald-100 print:text-[#0d5c43]">مؤسسة ومشاتل القادري الزراعية</p><h2 className="text-3xl font-black">طلب شراء</h2></div><div className="text-left"><p className="text-xs text-emerald-100 print:text-slate-500">رقم طلب الشراء</p><p className="mt-1 text-2xl font-black tracking-wider">{order.number}</p></div></div></div>
