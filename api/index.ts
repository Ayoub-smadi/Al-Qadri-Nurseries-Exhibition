...
router.post("/quotes", async (req, res) => {
  const { customerName, phone, items, notes } = req.body as { customerName?: string; phone?: string; items?: unknown[]; notes?: string };
...
router.put("/quotes/:id", async (req, res) => {
  if (!requireSession(req, res)) return;
  const { id } = req.params;
  const { items, discount, tax, status } = req.body as { items?: unknown; discount?: number; tax?: number; status?: string };
  try {
    await pool.query(
